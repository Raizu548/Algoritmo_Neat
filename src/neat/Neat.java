...
import data_structure.RandomSelector;
...
public class Neat {
    ...
    private double CP = 4; // es para ver la distancia, si un genoma pertenece a una especie

    // Se usa para los pesos de las conecciones
    private double WEIGHT_SHIFT_STRENGTH = 0.3; // Estos valores se pueden cambiar
    private double WEIGHT_RANDOM_STRENGTH = 1;  // Estos valores se pueden cambiar

    // Probabilidades de mutacion
    ...
    private double SURVIVORS = 0.8; // Porcentaje que van a sobrevivir -> 20% morira
    ...
    // Metodo que permite copiar ina connexion
    ...
    public void setReplaceIndex(NodeGene node1, NodeGene node2, int index){
        all_connections.get(new ConnectionGene(node1,node2)).setInnovation_number(index);
    }
    ...
    public NodeGene getNode(){
        NodeGene n = new NodeGene(all_nodes.size() + 1); // crea un nuevo nodo de inovacion
        all_nodes.add(n);
        return n;
    }
